...
class PositionRefineModel(RefineTemplate):
    ...
    def post_processing(self, data_dict):
        ...
        if self.model_cfg.POST_PROCESSING.get('GENERATE_RECALL', False) and 'gt_pos_trajectory' in data_dict:
        ...
        pred_dicts = {
            ...
            'pos_init_box': data_dict['pos_init_box'].cpu().numpy() if 'pos_init_box' in data_dict else None,
            'gt_pos_trajectory': data_dict['gt_pos_trajectory'].cpu().numpy() if 'gt_pos_trajectory' in data_dict else None
        }
