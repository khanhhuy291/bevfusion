...
class DetZeroTracker():
    ...
    def build_tracking(self, model_info_dict):
        ...
        self.model_cfg.TRACKING['DEVICE'] = device
        if 'DATA_ASSOCIATION' in self.model_cfg.TRACKING:
            self.model_cfg.TRACKING.DATA_ASSOCIATION['device'] = device
    ...
    def build_post_process(self, model_info_dict):
        ...
        device = self.model_cfg.get('DEVICE', 'cuda')
        self.model_cfg.POST_PROCESS['DEVICE'] = device
