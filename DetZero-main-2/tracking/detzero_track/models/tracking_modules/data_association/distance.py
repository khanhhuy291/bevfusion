...
def _cpu_bev_overlap(boxes_a, boxes_b):
    """Exact rotated rectangle overlap for the optional CPU reference path."""
    from shapely.geometry import Polygon
    def polygons(boxes):
        result = []
        for b in boxes.detach().cpu().numpy():
            xy = np.array([[-1, -1], [1, -1], [1, 1], [-1, 1]]) * b[3:5] / 2
            c, s = np.cos(b[6]), np.sin(b[6])
            result.append(Polygon(xy @ np.array([[c, s], [-s, c]]) + b[:2]))
        return result
    a, b = polygons(boxes_a), polygons(boxes_b)
    out = np.zeros((len(a), len(b)), dtype=np.float32)
    for i, pa in enumerate(a):
        for j, pb in enumerate(b):
            out[i, j] = pa.intersection(pb).area
    return out
...
def bev_overlap_gpu(boxes_a, boxes_b):
    ...
    N = boxes_a.shape[0]
    M = boxes_b.shape[0]
    if not boxes_a.is_cuda:
        return _cpu_bev_overlap(boxes_a, boxes_b)
    # bev overlap
    if N == 0 or M == 0:
...
def IoUBEV_dis_mat(boxes_a, boxes_b, gpu=True):
    ...
    M = boxes_b.shape[0]
    if not boxes_a.is_cuda:
        overlap = _cpu_bev_overlap(boxes_a, boxes_b)
        a = (boxes_a[:, 3] * boxes_a[:, 4]).detach().cpu().numpy()
        b = (boxes_b[:, 3] * boxes_b[:, 4]).detach().cpu().numpy()
        return overlap / np.maximum(a[:, None] + b[None, :] - overlap, 1e-6)
...
def IoU3D_dis_mat(boxes_a, boxes_b):
    ...
    if N == 0 or M == 0:
        ...
        from detzero_utils.ops.iou3d_nms.iou3d_nms_utils import boxes_iou3d_gpu
...
def GIoU3D_dis_mat(boxes_a, boxes_b):
    ...
    if N == 0 or M == 0:
        ...
        from detzero_utils.ops.iou3d_nms.iou3d_nms_utils import boxes_giou3d_gpu
