...
class PostProcessor():
    def __init__(self, processor_configs):
        self.device = processor_configs.get('DEVICE', 'cuda')
    ...
    def velocity_optimize(self, data_dict=None, config=None):
        ...
        for tk_id, tk_data in data_dict.items():
            ...
            for idx in range(process_len):
                dt = (tk_data['timestamp'][idx+1] - tk_data['timestamp'][idx]
                      if 'timestamp' in tk_data else 0.1)
                if dt <= 0:
                    raise ValueError('Track timestamps must increase')
                speed = (tk_data['boxes_global'][idx+1, :2] - tk_data['boxes_global'][idx, :2]) / dt
    ...
    def motion_classify(self, data_dict=None, config=None):
        ...
        for tk_id, tk_data in data_dict.items():
            ...
            if track_len < 2:
                ...
                track_box = torch.from_numpy(tk_data['boxes_global'][hit_index, :7]).float().to(self.device)
