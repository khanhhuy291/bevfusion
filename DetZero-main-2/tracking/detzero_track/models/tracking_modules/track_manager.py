...
class TrackManager():
    ...
    def forward(self, data_dict):
        ...
        # Optional timestamps are seconds, shared by forward and reverse passes.
        self.frame_timestamps = {}
        if any('timestamp' in data_dict[f] for f in frame_list):
            self.frame_timestamps = {str(f): float(data_dict[f]['timestamp']) for f in frame_list}
            times = np.array([self.frame_timestamps[str(f)] for f in frame_list])
        ...
        for _, frm_id in enumerate(frame_list):
            ...
            for key, val in frm_tk_data.items():
                ...
                if self.frame_timestamps:
                    tk_result[key]['timestamp'].append(self.frame_timestamps[str(frm_id)])
        ...
        if self.modules_dicts['reverse_tracking_config'].enable:
            ...
            for idx, frm_id in enumerate(frame_list[::-1]):
                frm_tk_data, reverse_tracks = self.reverse_tracking_module(
                    frm_id, data_dict[frm_id], frm_tracks.get(frm_id, {
                        'start': np.empty(0, dtype=int),
                        'boxes_global': np.empty((0, 9), dtype=np.float32),
                        'name': np.empty(0, dtype=str), 'score': np.empty(0),
                        'obj_ids': np.empty(0, dtype=int), 'num_points': np.empty(0),
                    }), reverse_tracks
                )
                for key, val in frm_tk_data.items():
                    for sub_key, sub_val in val.items():
                    ...
                    if self.frame_timestamps:
                        tk_result[key]['timestamp'] = np.insert(
                            tk_result[key]['timestamp'], 0, self.frame_timestamps[str(frm_id)])
    ...
    def predict_tracks(self, frm_id, tracks):
        ...
        for i, tk in enumerate(tracks):
            if getattr(self, 'frame_timestamps', None):
                dt = self.frame_timestamps[str(frm_id)] - self.frame_timestamps[str(tk.frame_id)]
                tk.delta_t = dt
                tk.F[:2, 3:5] = np.eye(2, dtype=np.float32) * dt
    ...
    def online_track_module(self, frame_id, det_data, tracks, track_id_count):
        ...
        for match_idx, match in enumerate(matched):
            ...
            source_indices[tracks[tk_idx].track_id] = int(det_data.get('source_index', np.arange(len(det_name)))[det_idx])

        for _, det_idx in enumerate(det_unmatch):
            source_indices[track_id_count] = int(det_data.get('source_index', np.arange(len(det_name)))[det_idx])
        ...
        if 'source_index' in det_data:
            for tid, info in track_output_data.items():
                info['source_index'] = source_indices.get(tid, -1)
    ...
    def reverse_tracking_module(self, frame_id, det_data, trk_data, tracks):
        ...
        for match_idx, match in enumerate(matched):
            ...
            source_indices[tracks[trk_idx].track_id] = int(det_data.get('source_index', np.arange(len(det_name)))[det_idx])
        ...
        for obj_idx, obj_id in enumerate(trk_data['obj_ids']):
            ...
            tracks.append(self.modules_dicts['filter_module'](
                ...
                delta_t=-abs(self.modules_dicts['filter_config']['delta_t'])))
    ...
    def overlap_track_merge(self, tracks):
        ...
        tk_boxes_cuda = torch.from_numpy(tk_boxes).float().to(self.model_cfg.get('DEVICE', 'cuda'))
