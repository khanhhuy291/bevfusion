...
def feature_decorator(features, num_voxels, coords, vx, vy, x_offset, y_offset, normalize_coords, use_cluster, use_center):
    # Only radar ONNX export needs this optional extension. Importing the
    # detection model must not require an extension absent from setup.py.
    try:
        from . import feature_decorator_ext
    except ImportError as exc:
        raise ImportError(
            "Radar ONNX export requires the optional feature_decorator_ext "
            "extension, which is not built by setup.py."
        ) from exc
